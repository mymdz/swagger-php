...
class MergeJsonContent
{
    public function __invoke(Analysis $analysis)
    {
        ...
        foreach ($annotations as $jsonContent) {
            ...
            $mustSkip = false;
            if (empty($response->content['application/json']) || $mustSkip = Analysis::mustExludeMethod($response->content['application/json']->schema->apiVersion, $jsonContent->apiVersion)) {

                if ($mustSkip) {
                    $response->content['application/json']->schema->skip();
                }
